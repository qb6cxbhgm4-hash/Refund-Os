...
function CustomersPage() {
  const rows = [["CUS-001", "James Kim", "james@example.com", "12 orders", "$482.00", "Active"], ["CUS-002", "Sarah Lee", "sarah@example.com", "8 orders", "$316.00", "Active"], ["CUS-003", "Mike Park", "mike@example.com", "23 orders", "$1,247.00", "VIP"], ["CUS-004", "Emily Choi", "emily@example.com", "3 orders", "$89.00", "New"], ["CUS-005", "Daniel Yoon", "daniel@example.com", "0 orders", "$0.00", "Inactive"]];
  return <Card title={`Customers (${rows.length})`} right={<Btn primary slim>Export</Btn>} flush>
    <table><thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Orders</th><th>Total Spent</th><th>Status</th></tr></thead>
      <tbody>{rows.map(([id, n, e, o, t, s]) => <tr key={id}><td style={{ fontSize: 12, color: "#8c9196" }}>{id}</td><td style={{ fontWeight: 500 }}>{n}</td><td style={{ fontSize: 12, color: "#6d7175" }}>{e}</td><td>{o}</td><td style={{ fontWeight: 600 }}>{t}</td><td><Badge c={s === "VIP" ? "i" : s === "Active" ? "s" : s === "New" ? "w" : "c"}>{s}</Badge></td></tr>)}</tbody></table>
  </Card>;
}

function RefundsPage() {
  const rows = [["RF-1024", "James K.", "Airpod Case", "Defective", "$15.00", "Pending"], ["RF-1023", "Sarah L.", "Phone Holder", "Wrong item", "$12.00", "Approved"], ["RF-1022", "Mike P.", "USB Cable", "Changed mind", "$8.00", "Approved"], ["RF-1021", "Emily C.", "Screen Film", "Defective", "$5.00", "Denied"], ["RF-1020", "Daniel Y.", "Charger", "Not working", "$28.00", "Pending"]];
  return <Card title={`Refund Requests (${rows.length})`} right={<Badge c="w">2 pending</Badge>} flush>
    <table><thead><tr><th>ID</th><th>Customer</th><th>Product</th><th>Reason</th><th>Amount</th><th>Status</th></tr></thead>
      <tbody>{rows.map(([id, c, p, r, a, s]) => <tr key={id}><td style={{ fontSize: 12, color: "#8c9196" }}>{id}</td><td style={{ fontWeight: 500 }}>{c}</td><td>{p}</td><td style={{ fontSize: 12, color: "#6d7175" }}>{r}</td><td style={{ fontWeight: 600 }}>{a}</td><td><Badge c={s === "Pending" ? "w" : s === "Approved" ? "s" : "c"}>{s}</Badge></td></tr>)}</tbody></table>
  </Card>;
}
...
function DiscountsPage() {
  const rows = [["SAVE10", "10% off", "All products", "Active", "34 used"], ["FREESHIP", "Free shipping", "Orders > $50", "Active", "128 used"], ["SUMMER25", "25% off", "Summer collection", "Expired", "67 used"], ["WELCOME", "$5 off first order", "New customers", "Active", "12 used"]];
  return <Card title={`Discount Codes (${rows.length})`} right={<Btn primary slim>Create discount</Btn>} flush>
    <table><thead><tr><th>Code</th><th>Type</th><th>Applies to</th><th>Status</th><th>Usage</th></tr></thead>
      <tbody>{rows.map(([code, type, applies, status, usage]) => <tr key={code}><td style={{ fontWeight: 600, fontFamily: "monospace", fontSize: 12 }}>{code}</td><td>{type}</td><td style={{ fontSize: 12, color: "#6d7175" }}>{applies}</td><td><Badge c={status === "Active" ? "s" : "c"}>{status}</Badge></td><td style={{ fontSize: 12, color: "#6d7175" }}>{usage}</td></tr>)}</tbody></table>
  </Card>;
}

function ShippingPage() {
  return <div style={{ animation: "fadeIn .25s ease" }}><Card title="Shipping Zones"><div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
    {[["Domestic", "Standard 3-5 days", "$4.99", "Active"], ["Domestic", "Express 1-2 days", "$12.99", "Active"], ["International", "Standard 7-14 days", "$18.99", "Active"], ["International", "Express 3-5 days", "$34.99", "Inactive"]].map(([zone, method, rate, status], i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px", background: "#f6f6f7", borderRadius: 8 }}><div><div style={{ fontSize: 13, fontWeight: 500 }}>{method}</div><div style={{ fontSize: 11, color: "#8c9196" }}>{zone}</div></div><div style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ fontSize: 14, fontWeight: 700 }}>{rate}</span><Badge c={status === "Active" ? "s" : "c"}>{status}</Badge></div></div>)}
  </div></Card></div>;
}

function BillingPage() {
  return <div style={{ maxWidth: 560, animation: "fadeIn .25s ease" }}><Card title="Billing & Plan">
    <div style={{ background: "#e4f5ee", border: "1px solid #b4dfca", borderRadius: 8, padding: 16, marginBottom: 20 }}><div style={{ fontSize: 11, color: "#004c3f", fontWeight: 500, textTransform: "uppercase", marginBottom: 4 }}>Current plan</div><div style={{ fontSize: 22, fontWeight: 700, color: "#008060" }}>Growth</div><div style={{ fontSize: 12, color: "#6d7175", marginTop: 4 }}>$69/month - 300 AI decisions - Billed monthly</div></div>
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>{[["Next billing date", "Apr 15, 2025"], ["Payment method", "Visa ending 4242"], ["AI decisions used", "187 / 300"], ["Stores connected", "1 / 3"]].map(([l, v]) => <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "10px 12px", background: "#f6f6f7", borderRadius: 6 }}><span style={{ fontSize: 13, color: "#6d7175" }}>{l}</span><strong style={{ fontSize: 13 }}>{v}</strong></div>)}</div>
    <div style={{ display: "flex", gap: 8 }}><Btn>Change plan</Btn><Btn>Update payment</Btn></div>
  </Card></div>;
}
...
/* ═══ APP ═══ */
const TITLES = { overview: "Home", products: "Products", detail: "Product Detail", alerts: "Orders", customers: "Customers", refunds: "Refunds", inventory: "Inventory", analytics: "Analytics", discounts: "Discounts", shipping: "Shipping", store: "Stores", billing: "Billing", settings: "Settings", help: "Help" };
const NAV = [
  ...
  { id: "billing", icon: "credit", label: "Billing", sec: "Settings" },
